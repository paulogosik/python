...
def relatorio_final():
    ...
    for linha in conteudo:
        ...
        if len(valores) > 4:
            ...
            aluno["N3"] = int(valores[4])
            aluno["N4"] = int(valores[5])
            aluno["Media"] = float((aluno["N1"] + aluno["N2"] + aluno["N3"] + aluno["N4"]) / 4)
    ...
    for aluno in alunos:
        print(f"{aluno['Matricula']} - {aluno['Nome']} | Média: {aluno['Media']} | {aluno['Situação']}")
